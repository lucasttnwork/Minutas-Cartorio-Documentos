-- ============================================================================
-- MIGRATION: Atualização dos Prompts para V2 - Catalogação Estruturada
-- Substitui "reescrita completa do documento" por "catalogação de dados por tipo"
-- ============================================================================

-- 1. Desativar todos os prompts v1
update public.agentes_especialistas_prompts
set ativo = false
where versao = 1;

-- ============================================================================
-- PESSOAIS (4 agentes) - V2
-- ============================================================================

-- 1. RG - Registro Geral / Carteira de Identidade - V2
...
-- 2. CNH - Carteira Nacional de Habilitacao - V2
insert into public.agentes_especialistas_prompts (
  agent_slug, versao, system_prompt, nome_exibicao, descricao, categoria, ativo
) values (
  'cnh',
...
-- ============================================================================
-- IMOBILIARIOS (5 agentes) - V2
-- ============================================================================

-- 5. Matricula de Imovel - V2
insert into public.agentes_especialistas_prompts (
  agent_slug, versao, system_prompt, nome_exibicao, descricao, categoria, ativo
) values (
  'matricula-imovel',
...
-- 9. Compromisso de Compra e Venda - V2
insert into public.agentes_especialistas_prompts (
  agent_slug, versao, system_prompt, nome_exibicao, descricao, categoria, ativo
) values (
...
-- ============================================================================
-- EMPRESARIAIS (2 agentes) - V2
-- ============================================================================

-- 10. Contrato Social - V2
insert into public.agentes_especialistas_prompts (
  agent_slug, versao, system_prompt, nome_exibicao, descricao, categoria, ativo
) values (
  'contrato-social',
...
-- Verificar que todos os 11 prompts v2 foram inseridos e estao ativos
do $$
declare
  v_count_v1_inativo integer;
  v_count_v2_ativo integer;
  v_count_total integer;
begin
  -- Contar v1 inativos
  select count(*) into v_count_v1_inativo
  from public.agentes_especialistas_prompts
  where versao = 1 and ativo = false;

  -- Contar v2 ativos
  select count(*) into v_count_v2_ativo
  from public.agentes_especialistas_prompts
  where versao = 2 and ativo = true;
  ...
  -- Validacoes
  if v_count_v1_inativo != 11 then
    raise exception 'Esperado 11 prompts v1 inativos, encontrado %', v_count_v1_inativo;
  end if;

  if v_count_v2_ativo != 11 then
    raise exception 'Esperado 11 prompts v2 ativos, encontrado %', v_count_v2_ativo;
  end if;

  if v_count_total != 22 then
    raise exception 'Esperado 22 prompts totais, encontrado %', v_count_total;
  end if;

  raise notice 'Migration V2 concluida com sucesso:';
  raise notice '  - % prompts v1 desativados', v_count_v1_inativo;
  raise notice '  - % prompts v2 ativos', v_count_v2_ativo;
  raise notice '  - % prompts totais', v_count_total;
end $$;
