// src/pages/ConferenciaOutorgantes.test.tsx
import { describe, it, expect, vi, beforeEach } from 'vitest';
import { render, screen, waitFor } from '@testing-library/react';
import { MemoryRouter, Routes, Route } from 'react-router-dom';
import ConferenciaOutorgantes from './ConferenciaOutorgantes';
...
describe('ConferenciaOutorgantes', () => {
  const defaultMinutaContext = {
    ...
    addPessoaNaturalOutorgante: vi.fn(),
    updatePessoaNaturalOutorgante: vi.fn(),
    removePessoaNaturalOutorgante: vi.fn(),
    addPessoaJuridicaOutorgante: vi.fn(),
    updatePessoaJuridicaOutorgante: vi.fn(),
    removePessoaJuridicaOutorgante: vi.fn(),
    addAdministradorOutorgante: vi.fn(),
    updateAdministradorOutorgante: vi.fn(),
    removeAdministradorOutorgante: vi.fn(),
    addProcuradorOutorgante: vi.fn(),
    updateProcuradorOutorgante: vi.fn(),
    removeProcuradorOutorgante: vi.fn(),
  };

  const renderWithRouter = (minutaId?: string) => {
    const path = minutaId ? `/minuta/${minutaId}/outorgantes` : '/minuta/outorgantes';
    return render(
      <MemoryRouter initialEntries={[path]}>
        <Routes>
          <Route path="/minuta/:id/outorgantes" element={<ConferenciaOutorgantes />} />
          <Route path="/minuta/outorgantes" element={<ConferenciaOutorgantes />} />
    ...
    it('should render form when data is loaded', () => {
      mockUseMinuta.mockReturnValue({
        ...
        currentMinuta: {
          ...
          outorgantes: {
            pessoasNaturais: [
              { id: '1', nome: 'Joao Silva', cpf: '123.456.789-00', camposEditados: [] },
            ],
            pessoasJuridicas: [],
          },
          outorgados: { pessoasNaturais: [], pessoasJuridicas: [] },
      ...
      expect(screen.getByText(/outorgante/i)).toBeInTheDocument();
    });
  });
});
